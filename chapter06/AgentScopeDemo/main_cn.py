...
class ThreeKingdomsWerewolfGame:
    ...
    async def witch_phase(self, killed_player: str):
        ...
        # 确定最终死亡玩家, 女巫如果没有 saved_player，则当晚被击杀的人就出局，同时，如果女巫还选择使用毒药，则再带走一人
    ...
    async def hunter_phase(self, shot_by_hunter: str):
        ...
        猎人阶段
        Args:
            shot_by_hunter: 被击杀的人
        """
        if not self.hunter:
            return None

        hunter_agent = self.hunter[0]
        # NOTE: 如果传入的被击杀的人是猎人自己（代表猎人被投票出局，因此，可以开枪带走一人）
    ...
    async def day_phase(self, round_num: int):
        ...
        async with MsgHub(
            ...
            # 投票阶段：set_auto_broadcast(False) 投票结果每个人的不同，不广播
